          ...
          <td>
            <div class="hover">
              <div class="hover-default">
                <div v-if="item.Roles.length == 0">N/A</div>
                <div v-else v-for="(role, index) in roles" :key="index">
                  <span v-if="item.Roles.includes(role.Id)">{{ role.Name }}</span>
                </div>
              </div>
              <div class="hover-show">
                <div class="form-check" v-for="(role, index) in roles" :key="index">
                  <input
                    :disabled="item.Type == 'miniauth' || role.Enable == false"
                    class="role_checkbox form-check-input"
                    type="checkbox"
                    :value="role.Id"
                    v-model="item.Roles"
                  />
    ...
    <nav aria-label="Page navigation">
      <ul class="pagination justify-content-center">
        <li class="page-item" :class="{ disabled: pageIndex === 0 }">
          <button class="page-link" @click.prevent="goToPage(pageIndex - 1)">
            {{ $t('Previous') }}
          </button>
        </li>
        <li
          ...
          v-for="(page, currentIndex) in computedPages"
          :key="currentIndex"
        >
          <button class="page-link" @click.prevent="goToPage(currentIndex)">
            {{ currentIndex + 1 }}
          </button>
        </li>
        <li
          class="page-item"
          :class="{ disabled: pageIndex >= Math.ceil(totalItems / pageSize) - 1 }"
        >
          <button class="page-link" @click.prevent="goToPage(pageIndex + 1)">
            {{ $t('Next') }}
          </button>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form v-if="isModalOpen">
                ...
                <div style="height: 100px; scroll-behavior: smooth; overflow-y: auto">
                  <div class="form-check" v-for="(role, index) in roles" :key="index">
                    <input
                      :disabled="editedUser.Type == 'miniauth' || role.Enable == false"
                      class="role_checkbox form-check-input"
                      type="checkbox"
                      :value="role.Id"
            ...
            <div class="modal-footer">
              ...
              <button type="button" @click="save(editedUser)" class="btn btn-primary">
                {{ $t('Save') }}
              </button>
            </div>
          </div>
